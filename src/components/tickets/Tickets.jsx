...
import { FaUser, FaRegCheckCircle, FaSpinner, FaTicketAlt, FaRegClock } from 'react-icons/fa'
import { BsFilterRight, BsPaperclip, BsQuestionCircleFill} from 'react-icons/bs'
  ...
  return (
    <div className='fixed top-0 left-0 w-screen h-screen z-[99] flex justify-center items-center'>
      ...
      <div className='relative z-[2] bg-white shadow-2xl rounded-lg md:max-w-[95vw] md:h-[95vh] overflow-hidden animate-fadeIn'>
        <PrismaZoom>
          <img src={img} alt="" className='md:max-w-[95vw] md:h-[95vh] object-contain' />
        </PrismaZoom>
      </div>
    </div>
  )
...
const Ticket = ({ticket, typeUser, getTickets, technicians = [], status, changeStatus, changeImg, changeTicket}) => {
    ...
    return (
        <li className="relative bg-white my-2 pr-4 rounded-md flex flex-col md:flex-row md:items-center justify-between hover:bg-[#fdfdfd] hover:shadow-md transition-all duration-150 border-2 border-gray-500" onClick={() => changeTicket(ticket?.Id)}>
              ...
              <div className="flex flex-col px-1 lg:flex-row lg:items-center lg:gap-2 text-gray-600">
                  ...
                  <div className="font-bold flex flex-col sm:flex-row items-center gap-1 sm:gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span title="Fecha de creación"><span className='block md:hidden'>Creación: </span>{dayjs(ticket?.FechaC).utc().format('DD MMM h:mm A')}</span>
                      </div>
                      {ticket?.FechaF && (
                      <>
          ...
          <div className="flex flex-col items-start justify-start gap-2 lg:flex-row lg:items-center lg:gap-8 px-4 pb-4">
            <div className="flex flex-col">
                <span>Ticket <b>{ticket?.Estado}</b>{ticket?.Estado.includes('PENDIENTE') && ticket?.FechaInicio && Duracion && (<><b>:</b> {Duracion}</>)}</span>
                <span>Tecnico: <b>{ticket?.Tecnico}</b></span>
            </div>
            {/* <Menu>
              <MenuHandler>
                <Button variant="filled" className='bg-black hover:shadow-gray-400'>Acciones</Button>
              </MenuHandler>
...
const Tickets = () => {
  ...
  return (
    <div className='py-4 px-8 relative overflow-hidden'>
      <div className="flex flex-wrap justify-between items-center w-full border-b-2 border-gray-500 sticky top-0 bg-gray-200 z-2 pb-3 md:pb-4">
      ...
      <div className="flex">
        <ul className={`overflow-y-auto h-[83.5vh] 3xl:h-[88vh] ${roleCode === 'TECNICO' && idSelected ? 'w-1/2': 'w-full'} transition-all duration-300`}>
          {tickets && tickets.length > 0 && tickets.sort((a, b) => order.indexOf(b.Estado) - order.indexOf(a.Estado)).map(ticket => (
            <Ticket changeTicket={handleIdSelected} ticket={ticket} typeUser={roleCode} key={ticket.Id} getTickets={getTickets} technicians={techs} status={modal} changeStatus={setModal} changeImg={setImg}/>
          )).reverse()}
        </ul>
        <div className={`${roleCode === 'TECNICO' && idSelected ? 'w-1/2' : 'w-0'} transition-all duration-300`}>
          <TicketWindow id={idSelected} changeTicket={handleIdSelected} technicians={technicians} getTickets={getTickets}/>
        </div>
      </div>
